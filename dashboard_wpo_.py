...
Dataset: Polinomial_Augmented_Dataset.xlsx
"""
...
@st.cache_data
def load_data():
    xls_path = "Polinomial_Augmented_Dataset.xlsx"
...
st.sidebar.markdown("Pusat Kendali Analisis Mesin Diesel WPO")
st.sidebar.markdown("---")
...
st.sidebar.markdown("---")
st.sidebar.markdown("**Informasi Dataset Augmentasi Polinomial**")
st.sidebar.info(
    f"- **Total Data**: {df.shape[0]} observasi\n"
    f"- **Ekstrapolasi Beban**: Dimulai dari 10%\n"
    f"- **Kualitas Data**: Terisolasi per-sumber\n"
)
...
st.markdown(
    "Eksplorasi data hasil kompilasi dan augmentasi **Polinomial Orde-2**. "
...
with tabs[0]:
    ...
    c2.metric("Titik Load Minimum", f"{df_f['load_pct'].min()}%")
...
# =====================================================================
# TAB 2: KURVA TERMODINAMIKA (LOAD VS SEMUA PARAMETER) - REQUESTED
# =====================================================================
with tabs[1]:
    st.subheader("📈 Analisis Termodinamika: Load (%) vs Parameter Kinerja & Emisi")
    st.markdown(
        "Halaman khusus ini memvisualisasikan bagaimana perubahan beban mesin secara terus-menerus "
        "(dari 10% hasil ekstrapolasi hingga 100%) memengaruhi parameter utama. "
        "Garis-garis dikelompokkan berdasarkan persentase campuran PPO."
    )
    ...
    df_curve = df_f[(df_f[paper_col] == selected_paper) & (df_f["speed_rpm"] == selected_rpm_curve)]
    ...
    cols = st.columns(2)
    for i, (col_id, title_name) in enumerate(target_kurva):
        if col_id in df_curve.columns:
            fig = px.line(
            ...
            fig.add_vline(x=10.0, line_dash="dash", line_color="red", annotation_text="Ekstrapolasi (10%)")
            cols[i % 2].plotly_chart(fig, use_container_width=True)

# =====================================================================
# TAB 3: 3D SURFACE PLOT (IDE KREATIF)
# =====================================================================
with tabs[2]:
    ...
    z_var = st.selectbox("Pilih Parameter Ketinggian (Sumbu Z):", 
                         ["power_kw", "bte_pct", "bsfc_kg_kwh", "nox_ppm", "co_ppm"], 
...
with tabs[4]:
    ...
    corr_data = df_f[corr_cols].dropna(how="all").corr()
    ...
    fig_corr.update_layout(title="Korelasi Antar Parameter Numerik", height=700)
...
with tabs[7]:
    ...
    with e1: x_axis = st.selectbox("Sumbu X:", all_numeric, index=all_numeric.index("power_kw") if "power_kw" in all_numeric else 0, format_func=lambda x: NUM_LABELS.get(x, x))
    with e2: y_axis = st.selectbox("Sumbu Y:", all_numeric, index=all_numeric.index("nox_ppm") if "nox_ppm" in all_numeric else 1, format_func=lambda x: NUM_LABELS.get(x, x))
    with e3: color_by = st.selectbox("Warna (Grup):", ["ppo_pct", "engine_model", "source_paper"], format_func=lambda x: NUM_LABELS.get(x, x))
